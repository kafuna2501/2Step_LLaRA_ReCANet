...
class MInterface(pl.LightningModule):
    ...
    def _extract_recommended_items(self, generated_text):
        """Parse recommended items from model output (item-only first, legacy formats supported)."""
        text = str(generated_text).strip()
        candidates = []

        # 1) Item-only mode: use the first non-empty line as primary candidate.
        if text:
            first_line = text.splitlines()[0].strip()
            if first_line:
                candidates.append(first_line)

        # 2) Backward compatibility: item: {x, y} / item: x
        ...
        for cand in candidates:
            cleaned = re.sub(r'^\s*item\s*[:：]\s*', '', str(cand), flags=re.IGNORECASE).strip()
            cleaned = cleaned.strip().strip('"').strip("'").strip()
            cleaned = cleaned.strip("{}｛｝").strip()
            name = self._normalize_item_name(cleaned)
            if name in self.all_item_name_set and name not in seen:
                normalized.append(name)
                seen.add(name)
        return normalized

    def _extract_first_item(self, generated_text):
        items = self._extract_recommended_items(generated_text)
        if not items:
            return ""
        return items[0]

    def _generate_explanation_from_item(self, generate_item):
        if not generate_item:
            return ""

        user_prompt = f"なぜ{generate_item}を選んだかを説明してください。"
        use_chat_template = getattr(self.hparams, "use_chat_template", False)
        system_prompt = getattr(self.hparams, "system_prompt", "")
        ...
        if use_chat_template:
            ...
            try:
                prompt_text = self.llama_tokenizer.apply_chat_template(
                    messages,
                    tokenize=False,
        ...
        new_ids = generate_ids[:, prompt_len:]
        generated = self.llama_tokenizer.batch_decode(
            new_ids,
        ...
        line = generated.splitlines()[0].strip()
        match = re.search(r'explanation\s*[:：]\s*(.+)', line, flags=re.IGNORECASE)
        if match:
